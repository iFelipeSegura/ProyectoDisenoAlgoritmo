# =========================================================
# 🚒 J.O.S.E-O FINAL PRO - FIXED VERSION
# =========================================================
...
def tiempo_ruta(G, ruta):
    ...
    for u, v in zip(ruta[:-1], ruta[1:]):
        ...
        edge = min(edges.values(), key=lambda x: x.get("travel_time", float("inf")))
        total += edge.get("travel_time", 0)
...
# =========================================================
# 🔥 ELIMINAR DUPLICADOS (FIX 3)
# =========================================================
def limpiar_rutas(ruta_main, rutas_alt):
    rutas = [ruta_main]

    for r in rutas_alt:
        if r["ruta"] != ruta_main["ruta"]:
            rutas.append(r)
...
def crear_mapa(G, rutas, cuarteles, origen, destino):
    ...
    if coords:
        folium.PolyLine(
            ...
            color="red",
            weight=8,
            tooltip=f"A* - {rutas[0]['tiempo']/60:.1f} min"
        ).add_to(m)
    ...
    if loc:
        ...
        if st.button("🚒 Calcular rutas"):
            ...
            if mejor:
                ...
                rutas_alt = rutas_alternativas(G, mejor, loc, k=3)

                # 🔥 FIX 3 aplicado aquí
                rutas = limpiar_rutas(ruta_main, rutas_alt)
...
if "rutas" in st.session_state:
    ...
    for i, r in enumerate(rutas):
        if i == 0:
            st.success(f"A* (mejor): {r['tiempo']/60:.2f} min")
        else:
